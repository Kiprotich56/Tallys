...
router.get("/staff/availability", async (req, res): Promise<void> => {
  ...
  const bookedSlots = await db.select({ timeSlot: appointmentsTable.timeSlot })
    ...
    .where(eq(appointmentsTable.staffId, staffId));

  const bookedTimes = new Set(bookedSlots.filter(b => b.timeSlot.startsWith(date)).map(b => b.timeSlot.split('T')[1] || b.timeSlot));
